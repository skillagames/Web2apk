...
c = c.replace(/npm install \$\{pluginsToInstall\.join\(' '\)\}/g, 'npm install ${pluginsToInstall.map(p => p.includes("@") && p.lastIndexOf("@") > 0 ? p : p + "@7").join(" ")}');
// Actually, earlier definition: let pluginsToInstall = ['@capacitor/splash-screen', '@capacitor/status-bar'];
// That would be easier to just change the array elements in the map.
